...
exports.Desugarer = void 0;
class Desugarer {
    static desugar(program) {
    ...
    static desugarBlock(block) {
        let term = { type: 'Lit', value: null, kind: 'boolean' };
    ...
    static desugarStatement(stmt, next) {
        switch (stmt.type) {
    ...
    static desugarExpression(expr) {
        switch (expr.type) {
            case 'Literal':
                return { type: 'Lit', value: expr.value, kind: expr.raw };
            case 'Identifier':
                if (expr.name === 'null')
